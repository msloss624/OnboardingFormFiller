...
from output.excel_generator import generate_rfi_excel, load_answers_from_excel, get_previous_sources
...
def init_state():
    defaults = {
        ...
        "excel_stats": None,
        # Baseline from previously saved file (for re-run workflow)
        "baseline_answers": [],
        ...
        for deal in st.session_state["search_results"]:
            ...
            if saved:
                ...
                with st.container():
                    ...
                    with dl_cols[1]:
                        if st.button("🔄 Continue from saved", key=f"continue_{deal.id}"):
                            baseline = load_answers_from_excel(latest)
                            prev_sources = get_previous_sources(latest)
                            st.session_state["baseline_answers"] = baseline
                            st.session_state["previous_sources"] = prev_sources
                            st.session_state["selected_deal"] = deal
                            st.session_state["deal_context"] = None
                            st.session_state["transcripts"] = None
                            st.session_state["extracted_answers"] = []
...
def step_gather():
    ...
    previous_sources = st.session_state.get("previous_sources", set())

    with st.expander(f"🎙️ Fireflies Transcripts ({len(transcripts)} found)", expanded=True):
        if previous_sources:
            st.caption("Previously processed transcripts are unchecked. New transcripts are checked by default.")
        if transcripts:
            for i, t in enumerate(transcripts):
                ...
                # Smart default: uncheck if this transcript was in a previous run
                default_checked = True
                if previous_sources:
                    source_label = f"Transcript: {t.title} ({date_str})"
                    if source_label in previous_sources:
...
def step_extracting():
    ...
    # Add transcripts (only those checked by the user)
    progress = st.progress(0, "Preparing sources...")
    selected_count = 0
    for i, t in enumerate(transcripts):
        if not st.session_state.get(f"transcript_{i}", True):
            continue
        ...
        selected_count += 1
        progress.progress(selected_count / max(len(transcripts), 1), f"Loaded transcript {selected_count}")
    ...
    if baseline:
        baseline_map = {a.field_key: a for a in baseline}
        for i, answer in enumerate(merged):
            base = baseline_map.get(answer.field_key)
            if not base:
                continue
            base_has_answer = base.confidence != Confidence.MISSING and base.answer
            new_has_answer = answer.confidence != Confidence.MISSING and answer.answer
            if base_has_answer and not new_has_answer:
                # Baseline filled, new extraction missed it — keep baseline
                merged[i] = base
            elif base_has_answer and new_has_answer:
                # Both have answers — keep baseline (already reviewed) unless
                # new is HIGH confidence and baseline was LOW
                if answer.confidence == Confidence.HIGH and base.confidence == Confidence.LOW:
                    pass  # keep new extraction
                else:
                    merged[i] = base
            # If baseline is MISSING and new found something, keep new (default)
